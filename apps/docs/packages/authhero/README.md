...
For multi-tenancy features including runtime fallback adapters (formerly "Main Tenant Adapter"), see the [@authhero/multi-tenancy](../multi-tenancy/) package.

::: warning Deprecated Features
The Main Tenant Adapter (`withMainTenantFallback`) has been deprecated and moved to `@authhero/multi-tenancy` as the Runtime Fallback Adapter. See the [migration guide](main-tenant-adapter.md).
:::
