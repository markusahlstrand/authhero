 ...
 * segment first, then the kind — never the operation id, which alone
...
export function buildEngineInstanceId(operation: {
  ...
  const kindBudget = Math.max(
    0,
    MAX_INSTANCE_ID_LENGTH - ("op-".length + 2 + operationId.length),
  );
  const kind = sanitizeSegment(operation.kind).slice(0, kindBudget);
