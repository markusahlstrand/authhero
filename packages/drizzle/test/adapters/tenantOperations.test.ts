...
describe("tenantOperations adapter", () => {
  ...
  it("lists with filters, status arrays, updated_before, and pagination", async () => {
    ...
    const byStatuses = await adapter.list({ status: ["pending", "running"] });
...
describe("tenantOperationEvents adapter", () => {
  let data: ReturnType<typeof getTestServer>["data"];
...
describe("non-control-plane adapters", () => {
  it("omits the control-plane adapters on tenant databases", () => {
    const { data } = getTestServer({ controlPlane: false });

    expect(data.tenantOperations).toBeUndefined();
    expect(data.tenantOperationEvents).toBeUndefined();
    expect(data.rollouts).toBeUndefined();
  });
});
