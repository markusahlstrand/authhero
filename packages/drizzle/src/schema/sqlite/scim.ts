...
export const scimConfigurations = sqliteTable(
  "scim_configurations",
  {
    connection_id: text("connection_id", { length: 255 }).primaryKey(),
  ...
  (table) => [index("scim_configurations_tenant_id_idx").on(table.tenant_id)],
);
...
export const scimTokens = sqliteTable(
  "scim_tokens",
  {
    token_id: text("token_id", { length: 64 }).primaryKey(),
    tenant_id: text("tenant_id", { length: 255 }).notNull(),
  ...
  (table) => [
    ...
    index("scim_tokens_tenant_hash_idx").on(table.tenant_id, table.token_hash),
  ],
);
...
export const scimExternalIds = sqliteTable(
  ...
  (table) => [
    primaryKey({ columns: [table.connection_id, table.user_id] }),
    uniqueIndex("scim_external_ids_connection_external_idx").on(
      table.connection_id,
      table.external_id,
    ),
    index("scim_external_ids_tenant_id_idx").on(table.tenant_id),
  ],
);
