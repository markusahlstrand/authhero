...
export async function invokeWebHook(
  ...
  // Sanitize the user payload so every customer-facing webhook (and the
  ...
  try {
    ...
    return { ok: response.ok, status: responseStatus, body: responseBody };
  } catch (error) {
...
export async function invokeHooks(
  ...
  for (const hook of hooks.filter(
    (h): h is Hook & { url: string } => "url" in h,
  )) {
    await invokeWebHook(ctx, hook, data);
  }
}
