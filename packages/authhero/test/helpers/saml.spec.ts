...
import { createSamlMetadata, SAMLMetadataParams } from "../../src/helpers/saml";
...
describe("createSamlMetadata", () => {
  it("should create SAML metadata with a single certificate", async () => {
    ...
    expect(parsedMetadata.EntityDescriptor["@_entityID"]).toBe("test-entity-id");
    expect(parsedMetadata.EntityDescriptor["@_xmlns"]).toBe("urn:oasis:names:tc:SAML:2.0:metadata");
    ...
    expect(idpDescriptor["@_protocolSupportEnumeration"]).toBe("urn:oasis:names:tc:SAML:2.0:protocol");
    ...
    expect(keyDescriptor.KeyInfo.X509Data.X509Certificate).toBe(certificate.cert);
    ...
    expect(singleLogoutServices[0]["@_Binding"]).toBe("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect");
    expect(singleLogoutServices[0]["@_Location"]).toBe("https://example.com/saml/sls");
    
    // HTTP-POST binding
    expect(singleLogoutServices[1]["@_Binding"]).toBe("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST");
    expect(singleLogoutServices[1]["@_Location"]).toBe("https://example.com/saml/sls");
    ...
    // HTTP-Redirect binding
    expect(singleSignOnServices[0]["@_Binding"]).toBe("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect");
    expect(singleSignOnServices[0]["@_Location"]).toBe("https://example.com/saml/acs");
    
    // HTTP-POST binding
    expect(singleSignOnServices[1]["@_Binding"]).toBe("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST");
    expect(singleSignOnServices[1]["@_Location"]).toBe("https://example.com/saml/acs");

    // Verify NameIDFormat
    const nameIdFormats = idpDescriptor.NameIDFormat;
    expect(nameIdFormats).toBeDefined();
    expect(nameIdFormats.length).toBe(3);
    expect(nameIdFormats[0]).toBe("urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress");
    expect(nameIdFormats[1]).toBe("urn:oasis:names:tc:SAML:2.0:nameid-format:persistent");
    expect(nameIdFormats[2]).toBe("urn:oasis:names:tc:SAML:2.0:nameid-format:transient");
    ...
    expectedAttributes.forEach((expectedAttr, index) => {
      ...
      expect(attributes[index]["@_FriendlyName"]).toBe(expectedAttr.friendlyName);
      expect(attributes[index]["@_NameFormat"]).toBe("urn:oasis:names:tc:SAML:2.0:attrname-format:uri");
      expect(attributes[index]["@_xmlns"]).toBe("urn:oasis:names:tc:SAML:2.0:assertion");
    });
  });
  ...
  it("should create SAML metadata with multiple certificates", async () => {
    ...
    expect(idpDescriptor.KeyDescriptor[0].KeyInfo.X509Data.X509Certificate).toBe(certificate1.cert);
    
    expect(idpDescriptor.KeyDescriptor[1]["@_use"]).toBe("signing");
    expect(idpDescriptor.KeyDescriptor[1].KeyInfo.X509Data.X509Certificate).toBe(certificate2.cert);
  });
  ...
  it("should escape special characters in URLs", () => {
    const testParams: SAMLMetadataParams = {
      entityId: "test&entity",
      assertionConsumerServiceUrl: "https://example.com/acs?param=value&other=test",
    ...
    expect(ssoService["@_Location"]).toBe("https://example.com/acs?param=value&other=test");
    expect(sloService["@_Location"]).toBe("https://example.com/sls?param=value&other=test");
  });
});
