...
- The PKCE verifier is generated by AuthHero's built-in OAuth2 client and stored on the `oauth2_state` code for the login session, so this points at the stored code being missing rather than at the verifier itself
- That code expires after 5 minutes — a slow or replayed callback won't find it
