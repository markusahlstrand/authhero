...
- 0ceae1d: Apply an organization's branding (logo, primary color, page background) on the universal login pages when the login runs with `organization`, overriding the tenant branding and theme like Auth0 does.
...
- Updated dependencies [9fefea9]
  ...
  - @authhero/widget@0.39.1
  - @authhero/proxy@0.11.1
  - @authhero/saml@0.5.13
...
- e5c33c6: Add `init({ unsafeAllowAzpCustomClaim: true })`, a transitional escape hatch
...
- 2354170: Trim whitespace when normalizing emails, not just lowercase.
...
- d222f28: Replace the deprecated `@react-email/components` dependency with the unified
...
- a748f96: Refresh grant reads auth-event data from the token, not the login session (#1258, stage 3 of #1255)
...
- a9d95db: Fix the universal login widget rendering off-centre on narrow viewports. A theme
...
- d8a0b0a: Scope the management PATCH user email-uniqueness check per connection
...
- 9e8a0ba: Fix mixed-case emails being stored on user accounts, and the duplicate accounts that followed.
...
- 0618ea7: Fix broken "verify your email" links after email/password signup
...
- c931ea2: Accept router condition groups without a `type: "and"` discriminator and resolve bare field paths (e.g. `user_metadata.birthdate`) against the user in form router conditions. Both shapes were emitted by the admin forms designer and silently never matched.
- a519115: Post-login forms now operate on the primary user of a linked account cluster.
...
- 2b9e5ca: Harden account-linking identifier semantics for merged users.
  ...
  - **Opt-in profile promotion.** New `copyProfileFields` option on the `account-linking` template fills _absent_ root profile fields on the primary from the secondary (e.g. a `birthdate` or phone the primary lacks) — fill-if-absent, never overwrite, and never an identifier or verification field. Off by default, matching Auth0 (a linked identity's own attributes otherwise stay under `identities[].profileData`).
...
- a7cca7b: Auto-generate a private signing key when provisioning a new WFP tenant, and fail
  ...
  - New exported `ensureSigningKey(keys, opts)` helper mints a create-if-missing
    RS256 `jwt_signing` key in a scope. It only mints when no _signable_ key
  ...
  - `resolveSigningKeys`' sign path now selects the newest _signable_ key (one
...
- 57eca12: Exclude Storybook `*.stories.tsx` fixtures from the package declaration build
...
- f1cbb4c: Record the login target (`redirect_uri`) on authentication logs. The
  ...
  `redirect_uri` field.
- 177c0a9: Signing-key rotation no longer revokes public-only control-plane keys. In a WFP
...
- 01abd78: Emit a `hook.post-user-update` outbox event when a user is updated (#1086).
...
- 8a7b5f1: Fix organization token issuance for global admins. The `admin:organizations`
...
- 3792e1a: Enrich failed `refresh_token` exchange logs (`fertft`) with the same identifying
...
- 730c0b0: Fix three pre-existing hook smells (issue #956):
...
- e52bbb9: Persist auth_strategy on login sessions everywhere it was being dropped:
