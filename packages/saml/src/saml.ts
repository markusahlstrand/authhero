...
export async function inflateRaw(
  ...
  const decompressedStream = new Blob([
    compressedData as unknown as ArrayBuffer,
  ])
    .stream()
    .pipeThrough(ds);
...
export async function parseSamlRequestQuery(samlRequestQuery: string) {
  ...
  const { samlRequestSchema } = await import("./types/saml");
...
export async function createSamlResponse(
  ...
  // Note: Using any type for the response JSON for now

  const samlResponseJson: any = [
    {
      "samlp:Response": [
        {
