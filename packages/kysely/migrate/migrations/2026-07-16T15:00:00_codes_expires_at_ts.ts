 ...
 * `expires_at_ts` + index that the session tables have (`login_sessions`,
 * `sessions` and `refresh_tokens` all carry one in the baseline), so even a
 * hand-rolled sweep was a full table scan.
 *
 * This sits on top of the baseline rather than inside it because the baseline
 * is a snapshot of production, and this landed after that snapshot was taken —
 * production has yet to receive it. Folding it in would make the baseline claim
 * to be a schema that has never existed anywhere.
 *
 * Order matters here:
 *
