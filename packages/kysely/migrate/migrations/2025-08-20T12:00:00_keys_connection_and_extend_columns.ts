...
export async function up(_: Kysely<any>): Promise<void> {
  ...
  // await db.schema
  //   .alterTable("keys")
  //   .modifyColumn("cert", "varchar(4096)")
  //   .execute();
  // // Extend pkcs7 column from varchar(2048) to varchar(4096)
  // await db.schema
  //   .alterTable("keys")
  //   .modifyColumn("pkcs7", "varchar(4096)")
  //   .execute();
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .alterTable("keys")
    .modifyColumn("cert", "varchar(2048)")
    .execute();

  // Revert pkcs7 column back to varchar(2048)
  await db.schema
    .alterTable("keys")
    .modifyColumn("pkcs7", "varchar(2048)")
    .execute();
