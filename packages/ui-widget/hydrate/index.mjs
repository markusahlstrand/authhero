...
const BUILD = /* authhero-widget */ { hotModuleReplacement: false, hydratedSelectorName: "hydrated", slotRelocation: true, state: true, updatable: true};
...
class AuthheroNode {
    ...
    renderErrors() {
        ...
        return errors.map((err) => (hAsync("span", { class: "error-text", part: "error-text", key: err.id ?? err.text }, err.text)));
    }
    /**
     * Render hint text.
     */
    renderHint(hint) {
        if (!hint)
            return null;
        return (hAsync("span", { class: "helper-text", part: "helper-text" }, hint));
    }
    // ===========================================================================
    // BLOCK Component Renderers
    // ===========================================================================
    renderDivider() {
        return hAsync("hr", { class: "divider", part: "divider" });
    }
    renderHtml(component) {
        return (hAsync("div", { class: "html-content", part: "html-content", innerHTML: component.config?.content ?? "" }));
    }
    renderImage(component) {
        const { src, alt, width, height } = component.config ?? {};
        if (!src)
            return null;
        return (hAsync("img", { class: "image", part: "image", src: src, alt: alt ?? "", width: width, height: height, loading: "lazy" }));
    }
    ...
    renderJumpButton(component) {
        return (hAsync("button", { type: "button", class: "btn btn-link", part: "button button-link", disabled: this.disabled, onClick: (e) => this.handleButtonClick(e, "jump", component.config.target_step) }, component.config.text ?? "Go"));
    }
    renderResendButton(component) {
        return (hAsync("button", { type: "button", class: "btn btn-link", part: "button button-link", disabled: this.disabled, onClick: (e) => this.handleButtonClick(e, "resend", component.config.resend_action) }, component.config.text ?? "Resend"));
    }
    // ===========================================================================
    ...
    renderLegalField(component) {
        ...
        return (hAsync("label", { class: "checkbox-wrapper", part: "checkbox-wrapper" }, hAsync("input", { type: "checkbox", part: "checkbox", name: component.id, checked: this.value === "true", required: component.required, disabled: this.disabled, onChange: this.handleCheckbox }), isHtml ? (hAsync("span", { class: "checkbox-label", part: "checkbox-label", innerHTML: text })) : (hAsync("span", { class: "checkbox-label", part: "checkbox-label" }, text))));
    }
    renderDropdownField(component) {
        const inputId = `input-${component.id}`;
        const errors = this.getErrors();
        const { options, placeholder } = component.config ?? {};
        return (hAsync("div", { class: "input-wrapper", part: "input-wrapper" }, this.renderLabel(component.label, inputId, component.required), hAsync("select", { id: inputId, class: { "input-field": true, "has-error": errors.length > 0 }, part: "input select", name: component.id, required: component.required, disabled: this.disabled, onChange: this.handleInput }, placeholder && (hAsync("option", { value: "", disabled: true, selected: !this.value }, placeholder)), options?.map((opt) => (hAsync("option", { value: opt.value, selected: this.value === opt.value, key: opt.value }, opt.label)))), this.renderErrors(), errors.length === 0 && this.renderHint(component.hint)));
    }
...
class AuthheroWidget {
    ...
    render() {
        ...
        if (!this._screen) {
            return (hAsync("div", { class: "widget-container" }, hAsync("div", { class: "error-message" }, "No screen configuration provided")));
        }
