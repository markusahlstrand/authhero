...
- 5cef6e6: Show the DATE field in the locale's own format: locale-correct separators (14.08.2026 for German/Nordic/Central European locales, 14-08-2026 for Dutch, 2026/08/14 for Japanese) and localized placeholder tokens (TT.MM.JJJJ, JJ/MM/AAAA, ÅÅÅÅ-MM-DD, …) instead of always DD/MM/YYYY with slashes.
...
- d9fcd8d: Restyle the "Last used" social-connection badge: the pill now sits on the button's top-right corner straddling the border (instead of overlapping the label), uses the theme primary color (`--ah-color-last-used` to override), and the matching button gets a highlighted border via a new `button-social-last-used` part. The widget demo gained a "Last used" Badge toggle to preview it.
...
- 9f120fe: Render long pick-one screens (e.g. tenant selection) as a scrollable, searchable list.
