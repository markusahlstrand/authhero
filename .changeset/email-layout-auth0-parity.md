...
All visual tokens are emitted as Liquid placeholders so tenants can re-skin without editing the template body: `{{ branding.button_text_color }}` and `{{ branding.button_border_radius }}` are now picked up by `PrimaryButton` (with `default:` filter fallbacks), `{{ signature.enabled }}` toggles the signature block, and `{{ footer.address }}` renders the address block when set.
