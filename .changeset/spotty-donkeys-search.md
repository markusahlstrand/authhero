...
The refresh-token grant now stamps `used_at` off the request's critical path (read and write both run under `waitUntil`), and the SSO-reuse path folds the stamp into the update it may already be doing. Both throttle to roughly one write per hour per session, so a client refreshing every few minutes costs about one `sessions` write an hour rather than one per exchange. The throttle reads the current stamp before writing rather than comparing-and-setting, so concurrent uses of the same session can occasionally write more than once in a window — that costs a redundant write, nothing more.
